...
block_count_dec(void)
{
    ...
    if (block_count == 0) {
	if (intrnl->misc.data.blocker_event)
	    erts_tse_set(intrnl->misc.data.blocker_event);
	return 1;
    }
...
erts_thr_progress_init(int no_schedulers, int managed, int unmanaged)
{
    ...
    intrnl->misc.data.blocker_event = NULL;
...
thr_progress_block(ErtsThrPrgrData *tpd, int wait)
{
    ...
    ASSERT(!intrnl->misc.data.blocker_event);

    if (wait) {
	event = erts_tse_fetch();
	erts_tse_reset(event);
	intrnl->misc.data.blocker_event = event;
    }
    if (tpd->is_managed)
...
erts_thr_progress_unblock(void)
{
    ...
    event = intrnl->misc.data.blocker_event;
    ASSERT(event);
    intrnl->misc.data.blocker_event = NULL;
