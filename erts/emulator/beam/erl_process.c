...
#define ERTS_MAIN_SCHED_SLEEP_SPINCOUNT 10000
#define ERTS_AUX_SCHED_SLEEP_SPINCOUNT 0

#define ERTS_SCHED_SLEEP_SPINCOUNT		\
  (erts_thr_get_main_status()			\
   ? ERTS_MAIN_SCHED_SLEEP_SPINCOUNT		\
   : ERTS_AUX_SCHED_SLEEP_SPINCOUNT)
...
#define ERTS_SYS_POLL_AUX_WORK_LIMIT 100
...
scheduler_wait(long *fcalls, ErtsSchedulerData *esdp, ErtsRunQueue *rq)
{
	...
	spincount = ERTS_SCHED_SLEEP_SPINCOUNT;
	...
	*fcalls = 0;
	...
	aux_work_limit = ERTS_SYS_POLL_AUX_WORK_LIMIT;
	...
	flgs = sched_spin_wait(ssi, ERTS_SCHED_SLEEP_SPINCOUNT);
	if (!(flgs & ERTS_SSI_FLG_WAITING)) {
	    ASSERT(!(flgs & ERTS_SSI_FLG_SLEEPING));
	    goto sys_woken;
	}
...
suspend_scheduler(ErtsSchedulerData *esdp)
{
	...
	blockable_aux_work:
		...
		flgs = sched_spin_suspended(ssi, ERTS_SCHED_SLEEP_SPINCOUNT);
		if (flgs == (ERTS_SSI_FLG_SLEEPING
			     | ERTS_SSI_FLG_WAITING
			     | ERTS_SSI_FLG_SUSPENDED)) {
