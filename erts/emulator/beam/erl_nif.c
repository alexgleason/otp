...
is_running_dirty_proc(Process *c_p)
{
    ...
    ASSERT(esdp);
    ASSERT(!c_p->scheduler_data || c_p->scheduler_data == esdp);
    ASSERT(!c_p->scheduler_data == !!ERTS_SCHEDULER_IS_DIRTY(esdp));
...
void erts_post_nif(ErlNifEnv* env)
{
...
#endif
    {
	ASSERT(!is_running_dirty_proc(env->proc));
	if (env->heap_frag == NULL) {
	...
	ASSERT(!ERTS_PROC_IS_EXITING(env->proc));
	env->exiting = 0;
	...
	ASSERT(is_running_dirty_proc(env->proc));
	ASSERT(is_running_dirty_proc(c_p));
...
{
    char *fatal_error_str;
    struct enif_msg_environment_t* menv = (struct enif_msg_environment_t*)msg_env;
    ErtsProcLocks rp_locks = 0, lc_locks = 0;
    Process* rp;
    Process* c_p;
    ErtsMessage *mp;
...
#ifndef ERTS_SMP
    if (!scheduler) {
	fatal_error_str = "enif_send: env==NULL on non-SMP VM";
	goto fatal_error;
    }
#endif
	...
	fatal_error_str = "enif_send: Invalid env for calling process";
	goto fatal_error;
    }
...
fatal_error:
    erts_exit(ERTS_ABORT_EXIT, fatal_error_str);
    return 0;
}
...
int enif_is_process_alive(ErlNifEnv* env, ErlNifPid *proc)
{
    ...
    rp = (scheduler > 0
	  ? erts_proc_lookup(proc->pid)
	  : erts_pid2proc_opt(c_p, 0, proc->pid, 0,
			      ERTS_P2P_FLG_INC_REFC));
    if (rp == NULL)
	return 0;
    if (scheduler <= 0)
