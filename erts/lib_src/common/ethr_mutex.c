...
do {								\
    ...
    if (res__ != 0)						\
	ethr_fatal_error__(ETHR_OP_MUTEX_Q_LOCK, res__);	\
} while (0)
#  define ETHR_MTX_Q_UNLOCK(L)					\
do {								\
    int res__ = pthread_mutex_unlock(L);			\
    if (res__ != 0)						\
	ethr_fatal_error__(ETHR_OP_MUTEX_Q_UNLOCK, res__);	\
} while (0)
...
ethr_cond_signal(ethr_cond *cnd)
{
    ...
    if (res != 0)
	ethr_fatal_error__(ETHR_OP_COND_SIGNAL, res);
}
...
ethr_cond_broadcast(ethr_cond *cnd)
{
    ...
    if (res != 0)
	ethr_fatal_error__(ETHR_OP_COND_BROADCAST, res);
}
...
ethr_cond_wait(ethr_cond *cnd, ethr_mutex *mtx)
{
    ...
    if (res != 0 && res != EINTR)
	ethr_fatal_error__(ETHR_OP_COND_WAIT, res);
    return res;
}
